...
describe('Frontend Firebase Integration', () => {
  ...
  it('should not have 404 errors for critical resources', async () => {
    ...
    const criticalFailures = failedRequests.filter(
      (url) =>
        !url.includes('favicon') && !url.includes('manifest') && !url.includes('browserconfig')
    );
  ...
  it('should display status dashboard correctly', async () => {
    ...
    await page.waitForSelector('[data-testid="status-dashboard"], .status-dashboard, h1', {
      timeout: 10000,
    });
    ...
    const hasProviderCards =
      (await page.locator('[class*="provider"], [class*="card"]').count()) > 0;
