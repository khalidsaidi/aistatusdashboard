          ...
          try {
            await Promise.race([
              ...
              new Promise((_, reject) =>
                setTimeout(() => reject(new Error('Delete timeout')), 1000)
              ),
            ]);
          } catch (error) {
  ...
  describe('High Volume Job Queuing', () => {
    it('should handle queuing 25 providers simultaneously', async () => {
      try {
        ...
        const providerIds = Array.from(
          ...
          (_, i) => `load-test-provider-${i}-${Date.now()}`
        );
    ...
    it('should handle batch queuing of 50 providers', async () => {
      try {
        ...
        const providerIds = Array.from(
          { length: providerCount },
          (_, i) => `batch-load-test-${i}-${Date.now()}`
        );
  ...
  describe('System Performance Under Load', () => {
    it('should maintain performance with multiple concurrent workers', async () => {
      try {
        ...
        const providerIds = Array.from(
          { length: 10 },
          (_, i) => `concurrent-test-${i}-${Date.now()}`
        );
    ...
    it('should handle backpressure gracefully when queue is full', async () => {
      ...
      try {
        ...
        for (let i = 0; i < 10; i++) {
          promises.push(
            smallQueue
              .queueProvider(`backpressure-test-${i}`, 0, { test: true })
    ...
    it('should maintain stable memory usage during extended operation', async () => {
      try {
        ...
        for (let i = 0; i < iterations; i++) {
          ...
          const providerIds = Array.from(
            { length: jobsPerIteration },
            (_, j) => `memory-test-${i}-${j}-${Date.now()}`
          );
          ...
          try {
            ...
            const metrics = await queue.getMetrics();
            console.log(
              `Iteration ${i + 1}: ${metrics.waitingJobs} waiting, ${metrics.activeJobs} active`
            );
