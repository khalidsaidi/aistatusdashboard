...
export class ThreadSafeCircuitBreaker {
  ...
  async shouldAllowRequest(
    ...
    return globalLockManager.withLock(`circuit:${key}`, async () => {
  ...
  async getStats(): Promise<{
    ...
    return globalLockManager.withLock('circuit:global', async () => {
      let openCircuits = 0;
  ...
  private async cleanup(): Promise<void> {
    try {
      return globalLockManager.withLock('circuit:cleanup', async () => {
        const now = Date.now();
        ...
        for (const [key, state] of Array.from(this.states.entries())) {
          if (state.state === 'closed' && 
              ...
              now - state.lastFailure > 24 * 60 * 60 * 1000) { // 24 hours
  ...
  private async forceCleanup(): Promise<void> {
    const states = Array.from(this.states.entries())
      .sort(([,a], [,b]) => a.lastFailure - b.lastFailure); // Sort by last activity
