...
export function checkIncidentResolution(
  ...
  const activeIncidents = Array.from(incidents.values()).filter(
    incident => incident.provider === current.id && 
  ...
  for (const incident of activeIncidents) {
    updateIncident(incident.id, {
      ...
      duration: Math.round(
        (new Date(current.lastChecked).getTime() - new Date(incident.startTime).getTime()) / (1000 * 60)
      ),
 ...
 */
function determineSeverity(currentStatus: string, previousStatus: string): 'low' | 'medium' | 'high' | 'critical' {
