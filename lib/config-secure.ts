...
function validateConfiguration(config: SecureConfig): void {
  ...
  for (const pattern of sensitivePatterns) {
    if (pattern.test(configStr)) {
      console.warn(`⚠️  Configuration contains pattern that should be in environment variables: ${pattern}`);
      // Don't throw in non-production to allow development
      if (config.environment === 'production') {
        throw new Error(`Configuration contains sensitive information that should be in environment variables`);
      }
    }
  }
...
class SecureConfigManager {
  ...
  getConfigSummary(): Record<string, any> {
    ...
    return {
      ...
      apiType: config.api.baseUrl.includes('localhost') ? 'local' : 
               config.api.baseUrl.includes('cloudfunctions') ? 'firebase' : 'other',
