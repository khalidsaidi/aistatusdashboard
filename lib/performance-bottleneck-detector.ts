...
export class PerformanceBottleneckDetector {
    ...
    if (metric && metric.status === 'running') {
      ...
      if (duration >= this.thresholds.operationTimeout) {
        ...
        this.createBottleneckAlert({
          ...
          recommendation: 'Add timeout protection, break into smaller operations, or implement async processing',
          timestamp: Date.now()
        });
  ...
  private analyzeOperation(metric: PerformanceMetric): void {
    ...
    if (metric.duration >= this.thresholds.blockingThreshold) {
      const severity: 'warning' | 'critical' | 'emergency' = 
        metric.duration >= this.thresholds.emergencyThreshold ? 'emergency' :
        metric.duration >= this.thresholds.operationTimeout ? 'critical' : 'warning';
  ...
  private checkMemoryLeakPattern(metric: PerformanceMetric): void {
    ...
    const recentSimilarOps = this.completedOperations
      .filter(op => 
        op.operation === metric.operation && 
        Date.now() - op.startTime < 60000 // Last minute
      ).length;
  ...
  private cleanupOldAlerts(): void {
    ...
    this.bottleneckAlerts = this.bottleneckAlerts.filter(alert => 
  ...
  getPerformanceStats(): {
    ...
    const slowOperations = this.completedOperations.filter(op => 
      op.duration && op.duration >= this.thresholds.blockingThreshold
    ).length;
    
    const averageTime = this.completedOperations.length > 0 ?
      this.completedOperations.reduce((sum, op) => sum + (op.duration || 0), 0) / this.completedOperations.length :
      0;
    
    const emergencyAlerts = this.bottleneckAlerts.filter(alert => 
