...
export function validateFirebaseConfig(config: FirebaseConfig): ValidationResult {
  ...
  if (config.apiKey === 'your_firebase_api_key_here' || 
  ...
  if (result.environment === 'production') {
    ...
    if (config.authDomain.includes('localhost') ||
        config.authDomain.includes('dev')) {
      result.errors.push('Production environment using development auth domain');
      result.isValid = false;
    }
    
    if (config.storageBucket.includes('dev') ||
        config.storageBucket.includes('test')) {
...
export function validateEnvironmentConsistency(): {
  ...
  switch (environment) {
    case 'production':
      ...
      break;
    case 'development':
      expectedProject = process.env.DEFAULT_FIREBASE_DEV_PROJECT_ID || process.env.FIREBASE_PROJECT_ID || 'demo-project';
      break;
    case 'test':
 ...
 */
export function generateFirebaseConfig(environment: 'development' | 'production' | 'test'): Partial<FirebaseConfig> {
  const baseConfig = {
    development: {
          projectId: process.env.DEFAULT_FIREBASE_DEV_PROJECT_ID || process.env.FIREBASE_PROJECT_ID || 'demo-project',
