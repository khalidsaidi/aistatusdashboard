...
const FIREBASE_PRICING = {
  ...
  functions: {
    invocations: 0.4 / 1000000, // $0.40 per million invocations
    gbSeconds: 0.0000025, // $0.0000025 per GB-second
    ghzSeconds: 0.00001, // $0.0000100 per GHz-second
    networkEgress: 0.12 / (1024 * 1024 * 1024), // $0.12 per GB
    freeQuotas: {
...
export class FirebaseQuotaOptimizer {
  ...
  calculateMonthlyCosts(usage: UsageMetrics): CostBreakdown {
    ...
    const firestoreReadsCharged = Math.max(
      0,
      usage.firestore.reads - FIREBASE_PRICING.firestore.freeQuotas.reads * 30
    );
    const firestoreWritesCharged = Math.max(
      0,
      usage.firestore.writes - FIREBASE_PRICING.firestore.freeQuotas.writes * 30
    );
    const firestoreDeletesCharged = Math.max(
      0,
      usage.firestore.deletes - FIREBASE_PRICING.firestore.freeQuotas.deletes * 30
    );
    const firestoreStorageCharged = Math.max(
      0,
      usage.firestore.storageGB -
        FIREBASE_PRICING.firestore.freeQuotas.storage / (1024 * 1024 * 1024)
    );
    const firestoreEgressCharged = Math.max(
      0,
      usage.firestore.networkEgressGB -
        FIREBASE_PRICING.firestore.freeQuotas.networkEgress / (1024 * 1024 * 1024)
    );
    ...
    costs.firestore.networkEgress =
      firestoreEgressCharged * FIREBASE_PRICING.firestore.networkEgress;
    costs.firestore.total =
      costs.firestore.reads +
      costs.firestore.writes +
      costs.firestore.deletes +
      costs.firestore.storage +
      costs.firestore.networkEgress;

    // Calculate Functions costs (subtract free quota)
    const functionsInvocationsCharged = Math.max(
      0,
      usage.functions.invocations - FIREBASE_PRICING.functions.freeQuotas.invocations
    );
    const functionsGbSecondsCharged = Math.max(
      0,
      usage.functions.gbSeconds - FIREBASE_PRICING.functions.freeQuotas.gbSeconds
    );
    const functionsGhzSecondsCharged = Math.max(
      0,
      usage.functions.ghzSeconds - FIREBASE_PRICING.functions.freeQuotas.ghzSeconds
    );
    const functionsEgressCharged = Math.max(
      0,
      usage.functions.networkEgressGB -
        FIREBASE_PRICING.functions.freeQuotas.networkEgress / (1024 * 1024 * 1024)
    );

    costs.functions.invocations =
      functionsInvocationsCharged * FIREBASE_PRICING.functions.invocations;
    costs.functions.compute =
      functionsGbSecondsCharged * FIREBASE_PRICING.functions.gbSeconds +
      functionsGhzSecondsCharged * FIREBASE_PRICING.functions.ghzSeconds;
    costs.functions.networkEgress =
      functionsEgressCharged * FIREBASE_PRICING.functions.networkEgress;
    costs.functions.total =
      costs.functions.invocations + costs.functions.compute + costs.functions.networkEgress;

    // Calculate Storage costs (subtract free quota)
    const storageStoredCharged = Math.max(
      0,
      usage.storage.storedGB - FIREBASE_PRICING.storage.freeQuotas.stored / (1024 * 1024 * 1024)
    );
    const storageDownloadsCharged = Math.max(
      0,
      usage.storage.downloadsGB -
        (FIREBASE_PRICING.storage.freeQuotas.downloads * 30) / (1024 * 1024 * 1024)
    );
    const storageUploadOpsCharged = Math.max(
      0,
      usage.storage.uploadOps - FIREBASE_PRICING.storage.freeQuotas.uploadOps * 30
    );
    const storageDownloadOpsCharged = Math.max(
      0,
      usage.storage.downloadOps - FIREBASE_PRICING.storage.freeQuotas.downloadOps * 30
    );
    ...
    costs.storage.operations =
      storageUploadOpsCharged * FIREBASE_PRICING.storage.uploadOps +
    ...
    const savingsFirestore =
      Math.min(usage.firestore.reads, FIREBASE_PRICING.firestore.freeQuotas.reads * 30) *
        FIREBASE_PRICING.firestore.reads +
      Math.min(usage.firestore.writes, FIREBASE_PRICING.firestore.freeQuotas.writes * 30) *
        FIREBASE_PRICING.firestore.writes +
      Math.min(usage.firestore.deletes, FIREBASE_PRICING.firestore.freeQuotas.deletes * 30) *
        FIREBASE_PRICING.firestore.deletes;

    const savingsFunctions =
      Math.min(usage.functions.invocations, FIREBASE_PRICING.functions.freeQuotas.invocations) *
        FIREBASE_PRICING.functions.invocations +
      Math.min(usage.functions.gbSeconds, FIREBASE_PRICING.functions.freeQuotas.gbSeconds) *
        FIREBASE_PRICING.functions.gbSeconds +
      Math.min(usage.functions.ghzSeconds, FIREBASE_PRICING.functions.freeQuotas.ghzSeconds) *
        FIREBASE_PRICING.functions.ghzSeconds;
      ...
      await globalLockManager.withLock(
        ...
        async () => {
          ...
          try {
            ...
            this.quotaMetrics.documentsPerBatch =
              (this.quotaMetrics.documentsPerBatch + batchOps.length) / 2;

            // Update usage tracking
            this.updateUsageMetrics('firestore', {
              writes: batchOps.filter((op) => op.operation === 'set' || op.operation === 'update')
                .length,
  ...
  private updateUsageMetrics(
    service: keyof UsageMetrics,
    updates: Partial<UsageMetrics[keyof UsageMetrics]>
  ): void {
