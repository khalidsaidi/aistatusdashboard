...
async function testAllScenarios() {
  ...
  try {
    ...
    const statusCount = await page
      .locator('[data-testid="provider-card"], .status-card, .provider-status')
      .count();
    ...
    await messageTextarea.fill(
      'This is a comprehensive test of the comments system in development environment. Testing all functionality before production deployment.'
    );
  ...
  const firebaseCalls = requests.filter(
    (req) => req.url.includes('cloudfunctions.net') || req.url.includes('/api/')
  );
