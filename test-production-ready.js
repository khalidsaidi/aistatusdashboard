...
const TEST_EMAIL = process.env.NODE_ENV === 'production' 
    ...
    return new Promise((resolve) => {
      ...
      discovery.on('close', (code) => {
        if (code === 0) {
          ...
          logTest('AI Provider Discovery Script', 'FAIL', `Exit code: ${code}, Error: ${errorOutput}`);
          resolve(false);
        }
      });
...
async function testLocalAPIRoutes() {
  ...
  const routes = [
    ...
    { path: '/subscribe-email', method: 'POST', body: { email: TEST_EMAIL, providers: ['openai'] } },
    { path: '/subscribe-push', method: 'POST', body: { endpoint: 'test', keys: { p256dh: 'test', auth: 'test' }, providers: ['openai'] } },
    { path: '/subscribe-webhook', method: 'POST', body: { url: 'https://webhook.site/test', providers: ['openai'] } }
  ];
...
async function testUnitTests() {
  ...
  try {
    ...
    return new Promise((resolve) => {
      const jest = spawn('npm', ['test', '--', '--watchAll=false', '--passWithNoTests', '--silent', '--maxWorkers=1', '--forceExit'], {
        stdio: 'pipe',
      ...
      jest.on('close', (code) => {
        ...
        if (code === 0) {
          ...
          if (testResults && suitesResults) {
            logTest('Unit Tests', 'PASS', `${suitesResults[1]} suites, ${testResults[1]} tests passed`);
          } else {
