    ...
    const checks = [
      {
        ...
        duration: 1,
        message: configSummary.apiConfigured ? 'Configuration loaded successfully' : 'Configuration failed to load'
      },
      {
        name: 'Firebase',
        status: config.firebase.projectId ? 'pass' : 'fail',
        duration: 1,
        message: config.firebase.projectId ? `Connected to ${config.firebase.projectId}` : 'Firebase connection failed'
      }
    ];
