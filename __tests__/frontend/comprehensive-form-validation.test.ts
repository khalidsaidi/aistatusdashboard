...
describe('📝 COMPREHENSIVE FORM VALIDATION - Every Input & Rule', () => {
  ...
  describe('📧 EMAIL ALERTS FORM - Complete Validation', () => {
    ...
    test('should validate email input field completely', async () => {
      ...
      for (const invalidEmail of invalidEmails) {
        ...
        const hasValidationError = await page.locator('text=/invalid/i, text=/error/i, .error, .invalid, [class*="error"]').count() > 0;
      ...
      for (const validEmail of validEmails) {
        ...
        const hasValidationError = await page.locator('.error:visible, .invalid:visible, [class*="error"]:visible').count() > 0;
    ...
    test('should validate provider selection completely', async () => {
      ...
      const deselectAllButton = page.locator('button:has-text("Deselect All"), button:has-text("None"), button:has-text("Clear")');
    ...
    test('should validate notification type selection', async () => {
      ...
      for (const type of notificationTypes) {
        const typeCheckbox = page.locator(`input[type="checkbox"][value*="${type.toLowerCase()}"], label:has-text("${type}") input`);
    ...
    test('should validate complete form submission', async () => {
      ...
      const hasErrors = await page.locator('.error:visible, .invalid:visible, [class*="error"]:visible').count() > 0;
      ...
      const hasSuccess = await page.locator('text=/success/i, text=/subscribed/i, .success, [class*="success"]').count() > 0;
      const isLoading = await page.locator('text=/loading/i, .loading, .spinner, [class*="loading"]').count() > 0;
  ...
  describe('🔔 WEB PUSH FORM - Complete Validation', () => {
    ...
    test('should validate notification type selection for push', async () => {
      ...
      for (const type of notificationTypes) {
        const typeElement = page.locator(`input[type="checkbox"][value*="${type.toLowerCase()}"], label:has-text("${type}")`);
  ...
  describe('🔗 WEBHOOKS FORM - Complete Validation', () => {
    ...
    test('should validate webhook form submission', async () => {
      ...
      const hasSuccess = await page.locator('text=/success/i, text=/added/i, .success, [class*="success"]').count() > 0;
      const isLoading = await page.locator('text=/loading/i, .loading, .spinner, [class*="loading"]').count() > 0;
