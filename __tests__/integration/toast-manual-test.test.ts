...
describe('Manual Toast Test - Direct Verification', () => {
  ...
  it('should manually trigger a toast notification and verify it appears', async () => {
    try {
      ...
      console.log(`Manual toast test completed. Found ${foundToasts} potential toast notifications.`);
  ...
  it('should check if the toast system is properly initialized', async () => {
    try {
      ...
      const reactCheck = await page.evaluate(() => {
        ...
        const hasToastCode = scripts.some(script => 
          script.textContent?.includes('toast') || 
          script.textContent?.includes('notification')
        );
