...
exports.subscribePush = (0, https_1.onRequest)({ cors: true, region: 'us-central1' }, async (req, res) => {
    ...
    try {
        ...
        v2_1.logger.info('Real push subscription created', { token: token.substring(0, 20) + '...', providers });
        ...
        try {
            ...
            v2_1.logger.info('Welcome push notification sent successfully', { token: token.substring(0, 20) + '...' });
        }
        catch (pushError) {
    ...
    async requeueFailedJobs(jobs) {
        ...
        for (const job of jobs) {
            if (job.retryCount < maxRetries) {
                ...
                setTimeout(() => {
                    this.enqueue(Object.assign(Object.assign({}, job), { retryCount: job.retryCount + 1, priority: Math.max(1, job.priority - 1) // Lower priority for retries
                     }));
                }, delay);
            }
            else {
