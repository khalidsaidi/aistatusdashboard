...
describe('Rate Limiter Module', () => {
    ...
    describe('checkRateLimit', () => {
        it('should allow requests within rate limit', () => {
            ...
            // Act
            const result = (0, rate_limiter_1.checkRateLimit)(identifier, config);
            // Assert
        ...
        it('should deny requests when rate limit exceeded', () => {
            ...
            const firstResult = (0, rate_limiter_1.checkRateLimit)(identifier, config);
            const secondResult = (0, rate_limiter_1.checkRateLimit)(identifier, config);
            const thirdResult = (0, rate_limiter_1.checkRateLimit)(identifier, config); // Should be denied
            // Assert
        ...
        it('should reset rate limit after window expires', () => {
            ...
            const firstResult = (0, rate_limiter_1.checkRateLimit)(identifier, config);
            const secondResult = (0, rate_limiter_1.checkRateLimit)(identifier, config); // Should be denied
            // Fast-forward past window
            currentTime += 1001; // 1001ms later
            const thirdResult = (0, rate_limiter_1.checkRateLimit)(identifier, config); // Should be allowed again
            // Assert
            expect(firstResult.allowed).toBe(true);
        ...
        it('should handle different identifiers separately', () => {
            ...
            // Act
            const result1 = (0, rate_limiter_1.checkRateLimit)(identifier1, config);
            const result2 = (0, rate_limiter_1.checkRateLimit)(identifier2, config);
            const result1Again = (0, rate_limiter_1.checkRateLimit)(identifier1, config); // Should be denied
            const result2Again = (0, rate_limiter_1.checkRateLimit)(identifier2, config); // Should be denied
            // Assert
        ...
        it('should maintain correct remaining count', () => {
            ...
            const deniedResult = (0, rate_limiter_1.checkRateLimit)(identifier, config);
    ...
    describe('getClientIdentifier', () => {
        it('should extract IP from x-forwarded-for header', () => {
            ...
            // Act
            const identifier = (0, rate_limiter_1.getClientIdentifier)(request);
            // Assert
    ...
    describe('createRateLimitHeaders', () => {
        it('should create correct rate limit headers', () => {
            ...
            // Act
            const headers = (0, rate_limiter_1.createRateLimitHeaders)(limit, remaining, resetTime);
            // Assert
    ...
    describe('RATE_LIMITS constants', () => {
        it('should have correct default rate limits', () => {
            // Assert
            expect(rate_limiter_1.RATE_LIMITS.status).toEqual({
                windowMs: 60000,
                maxRequests: 60
            });
            expect(rate_limiter_1.RATE_LIMITS.health).toEqual({
                windowMs: 60000,
                maxRequests: 30
            });
            expect(rate_limiter_1.RATE_LIMITS.provider).toEqual({
                windowMs: 60000,
                maxRequests: 120
            });
        });
        it('should have all required rate limit configurations', () => {
            // Assert
            expect(rate_limiter_1.RATE_LIMITS).toHaveProperty('status');
            expect(rate_limiter_1.RATE_LIMITS).toHaveProperty('health');
            expect(rate_limiter_1.RATE_LIMITS).toHaveProperty('provider');
            // Check structure
            Object.values(rate_limiter_1.RATE_LIMITS).forEach(config => {
