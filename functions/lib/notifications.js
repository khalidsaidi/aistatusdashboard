...
exports.subscribeEmail = (0, https_1.onRequest)(
...
exports.unsubscribeEmail = (0, https_1.onRequest)(
  { cors: true, region: 'us-central1' },
  async (req, res) => {
    if (req.method !== 'POST') {
      res.status(405).json({ error: 'Method not allowed' });
...
exports.subscribeWebhook = (0, https_1.onRequest)(
  { cors: true, region: 'us-central1' },
  async (req, res) => {
    if (req.method !== 'POST') {
      res.status(405).json({ error: 'Method not allowed' });
...
exports.sendTestNotification = (0, https_1.onRequest)(
  { cors: true, region: 'us-central1' },
  async (req, res) => {
    if (req.method !== 'POST') {
      res.status(405).json({ error: 'Method not allowed' });
    ...
    try {
      ...
      if (type === 'incident') {
        ...
        success = await emailService_1.emailService.sendStatusAlert(
          email,
          'OpenAI GPT-4',
          'degraded'
        );
