            ...
            try {
                ...
                if (provider.id === 'google-ai') {
                    ...
                    const hasActiveIncidents = Array.isArray(incidents) &&
                        incidents.length > 0 &&
                        incidents.some((incident) => !incident.end &&
    ...
    try {
        ...
        for (const currentStatus of currentStatuses) {
            ...
            if (previousStatus && currentStatus.status !== previousStatus.status) {
                ...
                try {
                    await (0, pushNotifications_1.sendStatusChangePushNotifications)(currentStatus.id, currentStatus.name, currentStatus.status, previousStatus.status);
                }
                catch (error) {
            ...
            batch.set(logRef, Object.assign(Object.assign({}, currentStatus), { timestamp: new Date(), statusChanged: previousStatus ? currentStatus.status !== previousStatus.status : false }));
        }
        // Commit all updates
        await batch.commit();
        ...
        for (const change of statusChanges) {
            if (((_a = change.previous) === null || _a === void 0 ? void 0 : _a.status) === 'operational' && change.provider.status !== 'operational') {
                // Create incident
                ...
                await incidentRef.set({
                    ...
                    statusPageUrl: (_b = PROVIDERS.find((p) => p.id === change.provider.id)) === null || _b === void 0 ? void 0 : _b.statusPageUrl,
                });
            ...
            if (((_c = change.previous) === null || _c === void 0 ? void 0 : _c.status) !== 'operational' && change.provider.status === 'operational') {
