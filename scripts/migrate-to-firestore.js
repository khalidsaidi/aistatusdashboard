...
async function migrateStatusResults() {
  return new Promise((resolve, reject) => {
    sqliteDb.all(
      'SELECT * FROM status_results ORDER BY checked_at DESC LIMIT 1000',
      async (err, rows) => {
        if (err) {
          reject(err);
          return;
...
async function createSampleData() {
  ...
  const sampleComments = [
    {
      content: 'OpenAI API is working great today!',
      author: 'Developer1',
      type: 'feedback',
      provider_id: 'openai',
    },
    {
      content: 'Having some issues with Anthropic responses',
      author: 'User2',
      type: 'issue',
