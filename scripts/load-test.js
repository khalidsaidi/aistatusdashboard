...
async function main() {
  ...
  const passed = stats.successRate >= MINIMUM_SUCCESS_RATE && 
                 stats.avgResponseTime <= MAXIMUM_AVG_RESPONSE_TIME;
  
  if (passed) {
    console.log('\n🎉 Load test PASSED!');
    console.log(`✅ Success rate (${stats.successRate.toFixed(2)}%) >= ${MINIMUM_SUCCESS_RATE}%`);
    console.log(`✅ Avg response time (${stats.avgResponseTime.toFixed(2)}ms) <= ${MAXIMUM_AVG_RESPONSE_TIME}ms`);
    process.exit(0);
  } else {
    console.log('\n💥 Load test FAILED!');
