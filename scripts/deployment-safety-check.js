...
function checkFirebaseConfig() {
  ...
  const configFiles = ['lib/config.ts', 'next.config.js', 'lib/firebase.ts'];
  ...
  if (fs.existsSync(envConfigPath)) {
    ...
    if (
      content.match(/(?<!DEFAULT_)FIREBASE_PROJECT_ID:\s*z\.string\(\)\.optional\(\)/) ||
    ...
    const hasProperEnvUsage =
      content.includes('process.env.FIREBASE_PROJECT_ID') ||
...
function checkErrorBoundaries() {
  ...
  const apiFiles = fs
    .readdirSync(path.join(__dirname, '../app/api'), { recursive: true })
...
function checkMonitoringAlerting() {
  ...
  const metricsFiles = ['lib/metrics.ts', 'lib/monitoring.ts'];

  const hasMetrics = metricsFiles.some((file) => fs.existsSync(path.join(__dirname, '..', file)));
...
async function runSafetyCheck() {
  ...
  const catastrophicIssues = results.issues.filter(
    (issue) => issue.severity === 'CATASTROPHIC'
  ).length;
